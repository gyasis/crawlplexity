...
export default function EditAgentPage({ params }: EditAgentPageProps) {
  ...
  const mapAgentType = (storedType: string): string => {
    // Map stored agent types to frontend types
    ...
    // Check if it's already a valid frontend type
    const validTypes = ['assistant', 'researcher', 'analyzer', 'orchestrator', 'creative', 'support']
    if (validTypes.includes(storedType)) {
      return storedType
    }
    
    // Try to map from stored type
    return typeMap[storedType] || 'assistant'
  }
  ...
  const loadAgent = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Loaded agent data:', agent) // Debug log
        
        setConfig({
          name: agent.name || '',
          description: agent.description || agent.metadata?.description || '',
          agent_type: mapAgentType(agent.agent_type || agent.metadata?.tags?.[0] || 'assistant'),
          ...
          capabilities: {
            expertise: Array.isArray(agent.capabilities?.expertise) 
              ? agent.capabilities.expertise 
              : agent.capabilities?.expertise 
                ? [agent.capabilities.expertise] 
                : [],
            taskTypes: Array.isArray(agent.capabilities?.taskTypes)
              ? agent.capabilities.taskTypes
              : agent.capabilities?.taskTypes
                ? [agent.capabilities.taskTypes]
                : [],
            complexity: agent.capabilities?.complexity || 'intermediate',
            contextAwareness: agent.capabilities?.contextAwareness || 5
          },
          metadata: {
            version: agent.metadata?.version || '1.0.0',
            author: agent.metadata?.author || '',
            tags: Array.isArray(agent.metadata?.tags)
              ? agent.metadata.tags
              : agent.metadata?.tags
                ? [agent.metadata.tags]
                : []
          ...
          advanced: {
            ...
            memoryEnabled: agent.config?.memoryEnabled ?? true,
            loggingLevel: agent.config?.loggingLevel || 'basic',
            rateLimiting: {
              enabled: agent.config?.rateLimiting?.enabled ?? true,
              requestsPerMinute: agent.config?.rateLimiting?.requestsPerMinute || 60
            }
          }
        })
      } else {
      ...
      alert('Failed to load agent. Redirecting to agents list.')
      router.push('/agents')
    } finally {
      setLoading(false)
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/agents?updated=${agentId}`)
      } else {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-600 dark:text-gray-400">Loading agent...</span>
        </div>
      </div>
    )
  }
